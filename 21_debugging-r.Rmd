...
command line. First you can set an editor breakpoint by cliking to the left of
...
The benefit to `recover()` over `browser()` for this case is that `recover()`
allows you to select where in the call stack you debug, not just the last call
where the error occurred. Often the issue is above The benefit to `recover()`
over using `options(error = browser)` is that you can browse on any of the call
stack, not just where the error occurred. Often the issue is most easily
diagnosed in calls higher on the call stack than immediately where the error
occurred.
...
```{block type = 'rmdinfo'}
[Try activity 1](activities/21_debugging-r-01.R) to use `traceback()`, breakpoints, `browser()` and `recover()`
to debug an commonly encountered error.
```
...
```{block type ="rmdinfo"}
[Try activity 2](activities/21_debugging-r-02.R) to explore using `trace()` with `at`.
```

## Debugging in RMarkdown documents

One special case where it can sometimes be more difficult to debug is an error
that occurs only when knitting a RMarkdown document.
...
However rarely an error will only occur when the code is being kitted. In this
...
Rmarkdown documents in any cases. The knit button opens a separate R process, so there is no
way to use an interactive debugger in that case.
```
